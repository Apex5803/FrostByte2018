...
public class VictorSPXFactory {
    ...
    // Create a VictorSPX with the default (out of the box) configuration.
...
//        talon.follow(master_id1);
    ...
    public static VictorSPX createVictor(int id, Configuration config) {
        ...
        victor.configOpenloopRamp(0, timeout);
        victor.configClosedloopRamp(0, timeout);
        victor.configPeakOutputForward(1, timeout);
        victor.configPeakOutputReverse(-1, timeout);
        victor.configNominalOutputForward(0, timeout);
        victor.configNominalOutputReverse(0, timeout);
        victor.configNeutralDeadband(0.04, timeout);
        victor.configVoltageCompSaturation(0, timeout);
        victor.configVoltageMeasurementFilter(32, timeout);
        victor.configSelectedFeedbackSensor(FeedbackDevice.QuadEncoder, 0, timeout);
        victor.configSelectedFeedbackCoefficient(1.0, 0, timeout);
        victor.configRemoteFeedbackFilter(id, RemoteSensorSource.Off, 0, timeout);
        victor.configSensorTerm(SensorTerm.Sum0, FeedbackDevice.QuadEncoder, timeout);
        victor.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_100Ms, timeout);
        victor.configVelocityMeasurementWindow(64, timeout);
        victor.configForwardLimitSwitchSource(LimitSwitchSource.Deactivated, LimitSwitchNormal.NormallyOpen, timeout);
//        victor.configReverseLimitSwitchSource(LimitSwitchSource.Deactivated, LimitSwitchNormal.NormallyOpen, timeout);
        victor.configForwardSoftLimitThreshold(0, timeout);
        victor.configReverseSoftLimitThreshold(0, timeout);
        victor.configForwardSoftLimitEnable(false, timeout);
        victor.configReverseSoftLimitEnable(false, timeout);
        victor.config_kP(0, 0.0, timeout);
        victor.config_kI(0, 0.0, timeout);
        victor.config_kD(0, 0.0, timeout);
        victor.config_kF(0, 0.0, timeout);
        victor.config_IntegralZone(0, 0, timeout);
        victor.configAllowableClosedloopError(0, 0, timeout);
        victor.configMaxIntegralAccumulator(0, 0.0, timeout);
        victor.configClosedLoopPeakOutput(0, 1.0, timeout);
        victor.configClosedLoopPeriod(0, 1, timeout);
        victor.configAuxPIDPolarity(false, timeout);
        victor.configMotionCruiseVelocity(0, timeout);
        victor.configMotionAcceleration(0, timeout);
        victor.configMotionProfileTrajectoryPeriod(0, timeout);
        victor.configSetCustomParam(0, 0, timeout);
//        victor.configPeakCurrentLimit(0, timeout);
//        victor.configPeakCurrentDuration(10, timeout);
//        victor.configContinuousCurrentLimit(0, timeout);
